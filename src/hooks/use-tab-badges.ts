...
import { obtenerAlarmasNoVistas, obtenerAvisos, obtenerAvisosNoLeidos, obtenerCorreosPendientes } from '@/lib/api';
 ...
 * Avisos cuenta solicitudes de encargo sin revisar + correos sin leer + avisos del
 * historial sin leer (ver historial-avisos.tsx). Comparte caché con las pantallas
...
export function useTabBadges() {
  ...
  const historial = useQuery({ queryKey: ['historial-avisos', 'no-leidos'], queryFn: obtenerAvisosNoLeidos });

  return {
    obrador: alarmas.data ?? 0,
    avisos: (avisos.data?.solicitudes.length ?? 0) + (correos.data?.correos.length ?? 0) + (historial.data ?? 0),
  };
}
