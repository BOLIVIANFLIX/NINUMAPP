...
import { obtenerAlarmas, obtenerAvisos } from '@/lib/api';
 ...
 * cuenta solicitudes de encargo sin revisar. Comparte caché con las pantallas reales
 * (misma queryKey) -- no dispara peticiones extra. */
export function useTabBadges() {
  ...
  return { obrador, avisos: avisos.data?.solicitudes.length ?? 0 };
}
